...
4. [🛠️ Использование](#4--использование)
   - [⌨️ Опции командной строки](#-опции-командной-строки)
   - [Использование готовой версии (рекомендуется)](#использование-готовой-версии-рекомендуется)
   - [Сборка из исходного кода](#сборка-из-исходного-кода)
   - [🎲 Генерация тестовых данных](#-генерация-тестовых-данных)
5. [🐛 Отладка](#5--отладка)
6. [🧠 Продвинутая логика и поведение](#6--продвинутая-логика-и-поведение)
7. [🚦 Коды завершения](#7--коды-завершения)
8. [🧑‍💻 Для разработчиков](#8--для-разработчиков)
   - [🧪 Стратегия тестирования](#-стратегия-тестирования)
   - [🚀 Автоматизация сборки и релизов](#-автоматизация-сборки-и-релизов)
9. [⚠️ Отказ от ответственности](#9--отказ-от-ответственности)
...
| `--verbose` | | Включает детальное логирование ошибок, включая полные трассировки стека исключений для отладки. |

### Использование готовой версии (рекомендуется)
...
## 6. 🧠 Продвинутая логика и поведение

Утилита использует несколько умных правил для обеспечения точной сортировки:

*   **Приоритет у наиболее точного совпадения**: Если имя файла соответствует нескольким папкам (например, файл `Отчет для Проекта X.pdf` может подойти папкам `Проект` и `Проект X`), утилита всегда выберет самое длинное, наиболее конкретное совпадение (`Проект X`). Это предотвращает неоднозначность и гарантирует, что файлы попадут в правильный подпроект.
*   **Сопоставление без учета пробелов**: Логика сопоставления игнорирует пробелы. Файл с именем `Отчет для ИванаИванова.pdf` будет корректно сопоставлен с папкой `Иван Иванов`.

## 7. 🚦 Коды завершения

Приложение использует стандартные коды завершения, что может быть полезно для автоматизации и написания скриптов.

| Код | Значение |
| :--- | :--- |
| `0` | Успех. Операция завершена без фатальных ошибок. |
| `1` | Ошибка. Операция была прервана из-за ошибки (например, неверные аргументы, отсутствующие директории или отмена пользователем). |

## 8. 🧑‍💻 Для разработчиков

Этот проект создан с упором на тестируемость и автоматизацию.

### 🧪 Стратегия тестирования
Проект имеет комплексный набор тестов, разделенный на два уровня:
*   **Модульные тесты**: Используют `xUnit` и `Moq` для тестирования основной логики `FileSorter` в полной изоляции от файловой системы и консоли.
*   **Интеграционные тесты**: Используют `xUnit` для сквозного запуска метода `Main`, тестируя парсинг аргументов командной строки, сценарии взаимодействия с пользователем и реальные операции с файловой системой во временной, изолированной среде.

### 🚀 Автоматизация сборки и релизов
Процесс релиза полностью автоматизирован с помощью **GitHub Actions**.
*   **Триггер**: Новый релиз собирается и публикуется автоматически при отправке в репозиторий нового тега, соответствующего шаблону `v*.*.*`.
*   **Кросс-платформенная сборка**: Рабочий процесс компилирует автономные исполняемые файлы для Windows, Linux и macOS.
*   **Автоматическое версионирование**: Номер версии извлекается из git-тега и внедряется в метаданные сборки, гарантируя, что версия, отображаемая командой `--help`, всегда будет точной.
*   **Заметки к релизу**: Заметки к релизу генерируются автоматически на основе истории коммитов с момента последнего тега.

## 9. ⚠️ Отказ от ответственности
